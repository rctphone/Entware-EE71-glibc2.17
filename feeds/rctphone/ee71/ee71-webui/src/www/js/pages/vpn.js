;(function() {
    ...
    function renderVpn(container) {
        container.innerHTML = '<h2>VPN</h2><div id="vpn-wg"></div><div id="vpn-ss"></div>';
        _loadAll();
        _wgTimer = setInterval(_refreshWg, 3000);
    ...
    // Silent refresh — update WG status without showing loading spinner
    function _refreshWg() {
        if (!document.getElementById('vpn-wg')) return;
    ...
    function _renderWg(el, status) {
        ...
        if (up) {
            if (hasPeerHandshake) {
                ...
                html += '<div class="vpn-status"><span class="status-dot on"></span><span class="text-success">Interface Up</span> <span class="text-muted text-small">\u2022 awaiting handshake</span></div>';
            } else {
                html += '<div class="vpn-status"><span class="status-dot on"></span><span class="text-success">Interface Up</span> <span class="text-muted text-small">\u2022 no peers</span></div>';
            }
