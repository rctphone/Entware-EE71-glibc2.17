...
/* globals to store upstream and the bridge interface */
char g_cur_iface[MAX_IFACE_LEN] ;
char g_bridge_iface[MAX_IFACE_LEN] ;
...
/* EE71: Periodic RA interval (seconds) for bridge interface */
#define RA_PERIODIC_INTERVAL 600

/* EE71: Cached raw RA packet for periodic resend and RS response on bridge */
static uint8_t cached_ra_buf[IOV_BUFF_LEN];
static int cached_ra_len;
static struct sockaddr_in6 cached_ra_src;
...
#define RADISH_BR_IFACE  "bridge"
#define RADISH_MAX_BRIDGE 10
...
static int radish_set_bridge_name() {
  int i;
  char br_name[MAX_IFACE_LEN];
  memset(br_name, 0, MAX_IFACE_LEN);
  strlcpy(br_name, RADISH_BR_IFACE, strlen(RADISH_BR_IFACE)+1);

  for (i = 0; i < RADISH_MAX_BRIDGE; i++) {
    br_name[strlen(RADISH_BR_IFACE)] = '0' + i;
    if (!if_nametoindex(br_name)) {
      strlcpy(g_bridge_iface, br_name, strlen(br_name)+1);
...
)
{
  ...
  if (RADISH_ERROR == radish_set_bridge_name()) {
    LOGE("Problem finding the bridge name. Radish cannot proceed.");
    exit(-1);
  }
...
)
{
  ...
  if ((!radish_create_counter_rules(tether_counters_chain,
    ...
    if (!radish_create_forward_rules(forward_chain,
      ...
      LOGD("Stats will be recorded on %s-%s interface", iface, g_bridge_iface);
      return RADISH_SUCCESS;
    }
  }
 ...
 * up this table and routes the ipv6 traffic based on this table.*/
      ...
      case 'i': {
                ...
                (!server_cfg->is_bridge_mode_on))
          {
            ...
            if(strncmp(cur_iface, RADISH_BR_IFACE, strlen(RADISH_BR_IFACE))) {
              memset(g_cur_iface,'0',sizeof(g_cur_iface));
              memcpy(g_cur_iface,cur_iface,strlen(cur_iface)+1);
            } else {
              memcpy(g_bridge_iface,cur_iface,strlen(cur_iface)+1);
              break;
            }
          ...
          else if (server_cfg->is_bridge_mode_on)
          {
            ...
            LOGD("Adding %s to %s",cur_iface, g_bridge_iface);
...
/*
 * EE71: Cache the raw RA packet after it's been processed
 * (RDNSS replaced, HW addr updated, checksum recalculated).
 * Called from radish_recv_proxy() when RA is sent on bridge.
 */
static void radish_cache_ra_packet
(
  struct parsed_nd *pnd
)
{
  if (!pnd || !pnd->iov || pnd->length <= 0 || pnd->length > IOV_BUFF_LEN)
    return;

  memcpy(cached_ra_buf, pnd->iov[0].iov_base, pnd->length);
  cached_ra_len = pnd->length;
  cached_ra_src = pnd->src;
  LOGD("Cached RA packet (%d bytes) for periodic resend", cached_ra_len);
}

/*
 * EE71: Send cached RA packet on the given interface.
 * Used for periodic RA resend and RS response on bridge.
 */
static void radish_send_cached_ra
(
  IfaceCfg *cfg
)
{
  struct buffer_item *buff;
  struct parsed_nd pnd;

  if (cached_ra_len <= 0 || !cfg)
    return;
  ...
  memcpy(buff->iov[0].iov_base, cached_ra_buf, cached_ra_len);
  buff->iov[0].iov_len = cached_ra_len;

  memset(&pnd, 0, sizeof(pnd));
  pnd.iov = buff->iov;
  pnd.length = cached_ra_len;
  pnd.ipv6hdr = (struct r_ip6_hdr *)buff->iov[0].iov_base;
  /* Destination: all-nodes multicast on this interface */
  pnd.dst.sin6_family = AF_INET6;
  inet_pton(AF_INET6, ALL_HOSTS_MGROUP, &pnd.dst.sin6_addr);
  pnd.dst.sin6_scope_id = cfg->iface_idx;
  /* Source: from cached RA, with this interface's scope */
  pnd.src = cached_ra_src;
  pnd.src.sin6_scope_id = cfg->iface_idx;

  LOGD("Sending cached RA (%d bytes) on %s", cached_ra_len, cfg->iface_name);
  radish_send_nd_packet(cfg->adv_socket, &pnd);

  radish_release_buffer(buff, __func__);
}

/*
 * EE71: Assign prefix::1 global IPv6 address to bridge interface.
 * Called when a new prefix is learned from carrier RA.
 */
static void radish_assign_bridge_global_addr
(
...
)
{
  ...
  struct in6_addr bridge_addr;

  if (!prefix || IN6_IS_ADDR_UNSPECIFIED(prefix) || '\0' == *g_bridge_iface)
    return;

  /* Construct prefix::1 */
  bridge_addr = *prefix;
  bridge_addr.s6_addr[15] = 1;
  inet_ntop(AF_INET6, &bridge_addr, ipaddr, MAX_IPV6_PREFIX);

  /* Flush existing global addresses on bridge */
  snprintf(cmd, MAX_CMD_LEN,
           "ip -6 addr flush dev %s scope global 2>/dev/null", g_bridge_iface);
  system(cmd);

  /* Add prefix::1/prefix_len */
  snprintf(cmd, MAX_CMD_LEN,
           "ip -6 addr add %s/%d dev %s", ipaddr, prefix_len, g_bridge_iface);
  LOGD("%s: %s", __func__, cmd);
  if (system(cmd) != 0)
  {
    LOGE("Unable to assign global IPv6 to %s", g_bridge_iface);
  }
  else
  {
    LOGD("Assigned %s/%d to %s", ipaddr, prefix_len, g_bridge_iface);
  }
}
...
)
{
  ...
  retval = snprintf(cmd,
                    MAX_CMD_LEN,
                    "ip -6 route del %s/%d dev %s scope link table %d",
                    ipaddr,
                    prefix_len,
                    g_bridge_iface,
...
)
{
  ...
  retval = snprintf(cmd,
                    MAX_CMD_LEN,
                    "ip -6 route add %s/%d dev %s scope link table %d",
                    ipaddr,
                    prefix_len,
                    g_bridge_iface,
                    ROUTE_TABLE_LOCAL_NETWORK);
...
)
{
  ...
  retval = snprintf(cmd,
                    MAX_CMD_LEN,
                    "ip -6 route %s %s dev %s scope link table %d",
                    operation ? RADISH_ADD : RADISH_DELETE,
...
)
{
  ...
  rlist_foreach(o, &pnd->parsed_options)
  {
    ...
    /* Here we are modifying the RDNSS address to that of bridge's Link layer
    ...
    if (popt->option->prefix_info.type == ND_OPT_TYPE_PREF_INFO)
    {
      ...
      else
      {
        ...
        /* EE71: Assign prefix::1 global address to bridge interface */
        radish_assign_bridge_global_addr(
              &ra_from_modem.global_ra.options->prefix_info.prefix,
              ra_from_modem.global_ra.options->prefix_info.prefix_length);
      }
    }
...
)
{
  ...
  if (0<radish_send_nd_packet(cfg->adv_socket, pnd))
  {
    ...
    /* EE71: Cache RA for periodic resend on bridge */
    if (pnd->icmphdr && pnd->icmphdr->type == ICMPV6_TYPE_ND_RA &&
        cfg->iface_name &&
        !strncmp(cfg->iface_name, RADISH_BR_IFACE, strlen(RADISH_BR_IFACE)))
    {
      radish_cache_ra_packet(pnd);
    }
  }
...
)
{
  ...
  if (cfg->iface_params.proxy)
  {
    ...
    /* EE71: Respond to RS from bridge clients with cached RA */
    if (pnd->icmphdr->type == ICMPV6_TYPE_ND_RS &&
        cfg->iface_name &&
        !strncmp(cfg->iface_name, RADISH_BR_IFACE, strlen(RADISH_BR_IFACE)) &&
        cached_ra_len > 0)
    {
      LOGD("RS on bridge — responding from RA cache");
      radish_send_cached_ra(cfg);
    }
...
#if defined FEATURE_DATA_LINUX_ANDROID || defined FEATURE_DATA_LINUX_LE
...
#endif /* FEATURE_DATA_LINUX_ANDROID || FEATURE_DATA_LINUX_LE */
...
)
{
  ...
  while (!cfg->exit)
  {
    ...
    /* EE71: For bridge interface, use periodic timeout to resend cached RA */
    if (is_bridge && cached_ra_len > 0)
    {
      struct timeval timeout;
      int ret;
      timeout.tv_sec = RA_PERIODIC_INTERVAL;
      timeout.tv_usec = 0;
      ret = select(cfg->nfds, &cfg->rfds, NULL, NULL, &timeout);
      if (ret == 0)
      {
        LOGD("Periodic RA timer expired on %s", cfg->iface_name);
        radish_send_cached_ra(cfg);
        continue;
      }
      else if (ret < 0)
      ...
      /* ret > 0: fall through to process events */
    }
    else
    {
...
)
{
  ...
  /* EE71: Flush global IPv6 address from bridge */
  if ('\0' != *g_bridge_iface)
  {
    snprintf(cmd, MAX_CMD_LEN,
             "ip -6 addr flush dev %s scope global 2>/dev/null", g_bridge_iface);
...
int main(int argc, char *argv[])
{
  ...
  /* setting up counter rules after g_cur_iface and g_bridge_iface are set */
#if defined FEATURE_DATA_LINUX_ANDROID
  if(radish_setup_ipv6_stat_counters(g_cur_iface,
                                     RADISH_TETHER_COUNTERS_CHAIN,
  ...
  rlist_foreach(cfg_lst, &server_cfg.iface_list) {
    IfaceCfg *icfg = IFACECFG(cfg_lst);
    if (icfg && icfg->iface_name){
#if defined FEATURE_DATA_LINUX_LE
      /* EE71: Only send RS on backhaul (WAN), not on bridge */
      if (!strstr(icfg->iface_name, server_cfg.backhaul))
        continue;
#endif
